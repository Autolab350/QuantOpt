...
Run the GEX calculator once at market open, then use cached data throughout the day.
...
9:30 AM (bell rings)
  └─ python gex_cache_manager.py --refresh (60 sec)
  └─ See: "Regime NEGATIVE, 83% confidence"
  └─ Set your bias for the day

9:35 AM - 4:00 PM
  └─ python run_gex.py (0.9 sec)
  └─ Check if setup still valid
  └─ Use GEX to filter bad setups
...
**Why this works:**
- Fetch live CBOE data ONCE at open (60 seconds)
- Cache it for the entire trading day
- All subsequent checks use cache (instant, no network overhead)
- Gamma exposure doesn't change drastically intraday
- 2 refreshes per day × 60 seconds = 2 minutes total overhead
